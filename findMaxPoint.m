function [max_point] = findMaxPoint(xs, ys, img_crop)
 ...
 % wczeœniej
    ...
    pa1 = img_crop(pa1_ind(1), pa1_ind(2));
    pa2_ind = [xs(end)+1, ys(end)-1];
    pa2 = img_crop(pa2_ind(1), pa2_ind(2));
    pa3_ind = [xs(end), ys(end)-1];
    pa3 = img_crop(pa3_ind(1), pa3_ind(2));
    pa4_ind = [xs(end)-1, ys(end)-1];
    pa4 = img_crop(pa4_ind(1), pa4_ind(2));
    pa5_ind = [xs(end)-1, ys(end)];
    pa5 = img_crop(pa5_ind(1), pa5_ind(2));
    pa6_ind = [xs(end)+1, ys(end)+1];
    pa6 = img_crop(pa6_ind(1), pa6_ind(2)); 
    pa7_ind = [xs(end)-1, ys(end)+1];
    pa7 = img_crop(pa7_ind(1), pa7_ind(2));
    pa8_ind = [xs(end), ys(end)+1];
    pa8 = img_crop(pa8_ind(1), pa8_ind(2));
    ...
    while is_changed == 1
        ...
        prev_pointsx_ind = find(xs(xs == max_point(1)));
